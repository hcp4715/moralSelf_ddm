...
rm('df.M.hddm','df.C.hddm','df.M.hddm_m','df.M.hddm_nm','df.C.hddm_val','df.C.hddm_id','df.M.ddm.v','df.C.ddm.v')
setwd(curDir)
...
# plot four scatter point.
# p1: goodself-goodother (d in matching and acc in id-based)
cor_p1 <- ggplot(df.v.sum_eff_all_w, aes(d_goodslf_goodoth, Id_ACC_goodslf_goodoth))+ 
          geom_point() + 
          labs(x = "Matching task (d prime)", y = "Identity-base Categorization task (accuracy)") +
          geom_smooth(method="lm")

# p2: goodself-goodother (RT in matching and RT in id-based)
# p3: badself-badother (d in matching and acc in id-based)
# p4: badself-badother (RT in matching and RT in id-based)

```

```{r preproc questionnair,echo=FALSE,results='hide',warning=FALSE, message=FALSE}
# load the data
df.quest1 <- read.csv("exp7_rep_Q4.csv",header = TRUE, sep = ',', stringsAsFactors=FALSE) %>%
      filter(session == 1)
df.quest2 <- read.csv("exp7_rep_Q5.csv",header = TRUE, sep = ',', stringsAsFactors=FALSE) %>%
      filter(session == 1)
df.quest <- merge(df.quest1,df.quest2,by = "subjID")
...
print(slfEsteemAlpha$total[2]) # 0.8593
slfEsteemOmega <- psych::omega(df.quest[,slfEsteemNames]) # omega 0.7455 0.9341
...
print(moralIdAlpha$total[2]) # 0.9178
moralIdOmega <- psych::omega(df.quest[,moralIdNames]) # omega 0.7583 0.9454
...
print(moralIdinAlpha$total[2]) # 0.8884
moralIdinOmega <- psych::omega(df.quest[,moralIdinNames]) # omega 0.7115 0.939
...
print(moralIdexAlpha$total[2]) # 0.844
moralIdexOmega <- psych::omega(df.quest[,moralIdexNames]) # omega 0.684 0.91
...
# load data from exp1
crossf_effect_scale_1 <- read.csv("D:\\HCP_cloud\\Exps\\P1_Pos_Self\\Exp_Behav_Moral_Asso\\Exp_7_share\\Results\\Traditional_Analysis\\MS_cross_taskeffect_wide_w_scale.csv",header = TRUE, sep = ',', stringsAsFactors=FALSE)
colnames(tmp) == colnames(crossf_effect_scale_1)
colnames(tmp)[colnames(tmp) == 'Sex'] <- 'Gender'
tmp <- tmp[,colnames(crossf_effect_scale_1)]
new_tmp <- rbind(crossf_effect_scale_1,tmp)
# write files to an upper-level folder
setwd(traDir)
write.csv(new_tmp,'MS_effect_w_scales_all.csv',row.names = F)
setwd(curDir)
```
...
Mplots(saveDir = traDir, curDir = curDir, expName = 'exp7_rep', df.M1.V.SDT_l,df.M1.V.sum_rt_acc_l)
      
```

```{r plot_categ_val_data,echo=FALSE,warning=FALSE, message=FALSE}
CAplots(saveDir = traDir, curDir = curDir,expName = 'exp7_rep', task = 'id', df.C1.V.sum_rt_acc_l)
CAplots(saveDir = traDir, curDir = curDir,expName = 'exp7_rep', task = 'val', df.C1.V.sum_rt_acc_l)
```

...
df.L.ddm.v <- df.L[!(df.L$Subject %in% c(procFailureSub,excld.sub)),]   # exclude the invalid subjects


df.T.ddm.v <- df.T[!(df.T$Subject %in% c(procFailureSub,excld.sub)),]
df.T.ddm.v <- df.T.ddm.v[!(df.T.ddm.v$Subject %in% excld.sub.T),]
df.T.ddm.v <- df.T[df.T$Task != 'importance',]  # exclude the results from importance task

#df.L1 <- df.L1[!(df.L1$Subject %in% procFailureSub),]
#df.T1 <- df.T1[!(df.T1$Subject %in% procFailureSub),]

#df.L1.valid <- df.L1.fm[!(df.L1.fm$Subject %in% excld.sub),]
#df.T1.valid <- df.T1[!(df.T1$Subject %in% excld.sub),]
#df.T1.valid <- df.T1.valid[!(df.T1.valid$Subject %in% excld.sub.T),]

df.L.hddm <- subset(df.L.ddm.v, ACC ==1 | ACC == 0)  # remove non-response trials or pressing wrong key
df.T.hddm <- subset(df.T.ddm.v, ACC ==1 | ACC == 0) 
...
df.L1.V.SDT_w$Subject <- factor(df.L1.V.SDT_w$Subject)  # make the subject "factor"
...
e7_L.d.t.good_bad_slf <- t.test(df.L1.V.SDT_ww$Self_Good,df.L1.V.SDT_ww$Self_Bad,paired = TRUE)     # t-test
df.L1.V.SDT_ww$good_bad_slf <- df.L1.V.SDT_ww$Self_Good - df.L1.V.SDT_ww$Self_Bad                   # calculate the differences
e7_L.d.t.good_bad_slf.CI <- bootES(df.L1.V.SDT_ww$good_bad_slf,R = 20000, effect.type = "cohens.d") # calculate the cI using BootES
...
e7_T.rt_anova <- ezANOVA(df.T1.V.RT,dv = RT, wid = Subject, within=.(Task,Morality,Identity),
                         within_full=.(Task,Identity,Morality), type=3)

e7_T.rt_anova.Id_task <- ezANOVA(df.T1.V.RT_Id_task,dv = RT, wid = Subject, 
                                 within=.(Morality,Identity),within_full=.(Identity,Morality), type=3)

e7_T.rt_anova.val_task <- ezANOVA(df.T1.V.RT_Val_task,dv = RT, wid = Subject,
                                  within=.(Morality,Identity),within_full=.(Identity,Morality), type=3)
